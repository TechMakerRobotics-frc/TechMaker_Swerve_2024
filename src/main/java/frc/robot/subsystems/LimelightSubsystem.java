...
public class LimelightSubsystem extends SubsystemBase {
  ...
  public Pose2d getBotpose() {
    ...
    if (result.length > 0.0) {
      return new Pose2d(
          new Translation2d(result[0], result[1]), new Rotation2d(Math.toRadians(result[5])));
    }
    return new Pose2d();
  }
  ...
  // public Translation2d getCurrentAprilTag() { // gets the april tag the limelight is currently
  // seeing
  //     return LimelightHelper.getAprilTagCoordinates((int) getTargetID()); // this isn't the
  // closest, it's just the one we're seeing
  // }
